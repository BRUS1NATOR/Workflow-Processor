...
namespace WorkflowProcessor.Activities.Gateways
{
    public class ParallelExclusiveGateway<T> : WorkflowElement, IGateway
    {
        ...
        public override async Task<ActivityExecutionResult> ExecuteAsync(IWorkflowInstance instance)
        {
            ...
            {
                return await Task.FromResult(ActivityExecutionParallelResultWithValue.Next(instance, null));
            }
            var result = _conditon.Invoke(instance);
            return await Task.FromResult(ActivityExecutionParallelResultWithValue.Next(instance, result));
        }
    }
